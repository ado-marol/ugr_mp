...
void Kmer::toLower()
{
    int size = _text.size(); // To prevent problems with signed/unsigned ints
    
    for (int i = 0; i < size; i++) { // We run through our Kmer, converting to
                                     // lowercase any letter which isn't already
        
        _text.at(i) = std::tolower(_text.at(i));
    }
...
void Kmer::toUpper()
{
    ...
    for (int i = 0; i < size; i++) { // We run through our Kmer, converting to
                                     // uppercase any letter which isn't already
...
void Kmer::normalize(const std::string& validNucleotides)
{
    ...
    auxiliary_kmer.toUpper();    // Efficiently make use of the function.
