...
int main(int argc, char* argv[]) {
    ...
    // If the length of each kmer is larger than the size of the genetic 
    // string sequence, we crop it to be equal to the sequence size, adding 
    // 1 to compensate for the fact that string begins in position 0.
    if (k > sequence_size)
        k = sequence_size +1;
    
    // If the total number of kmers (sequence_size - k + 1) is larger than 
    // the dimension of our array, we make sure that it will be equal to it.
    // We follow this simple mathematical process:
    /*
        sequence_size - k + 1 = DIM_ARRAY_KMERS =>
        sequence_size = DIM_ARRAY_KMERS + k - 1
    */
    
    if ((sequence_size - k + 1) > DIM_ARRAY_KMERS) 
        sequence_size = DIM_ARRAY_KMERS + k - 1;
